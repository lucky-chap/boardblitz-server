...
export async function claimAbandoned(this: Socket, type: "win" | "draw", callback: (response: any) => void) {
  ...
  if (
    ...
    !game.white ||
    !game.black ||
    (game.white.id !== this.request.session.user.id && game.black.id !== this.request.session.user.id)
  ) {
  ...
  if (
    (game.white &&
      ...
      (game.black?.connected || Date.now() - (game.black?.disconnectedOn as number) < 50000)) ||
    (game.black &&
  ...
  try {
    ...
    if (newMove) {
      ...
      if (chess.isGameOver()) {
        ...
        const winnerName =
          reason === "checkmate" ? (winnerSide === "white" ? game.white?.name : game.black?.name) : undefined;
        if (reason === "checkmate") {
